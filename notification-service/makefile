...
containerID := $(shell docker ps -a -q --filter ancestor=ahmedheshamafify/notification-service:latest)
...
#===================  docker  ===========================
docker-build: build
	docker build -t ahmedheshamafify/notification-service:latest .


docker-run: docker-build
	docker run -p 8080:8080 ahmedheshamafify/notification-service:latest
...
docker-push: docker-login docker-build
	docker push ahmedheshamafify/notification-service:latest

docker-bash:
	docker exec -ti ahmedheshamafify/notification-service:latest /bin/sh
...
k8s-persistence-create:
	...
	echo "kubectl run -it --rm --image=mysql:5.6 --restart=Never mysql-client -- mysql -h accontservice-db -ppassword -e 'create database db_example;'"
...
k8s-info:
	...
	kubectl get pods
	echo "You can also check DB via:\n kubectl run -it --rm --image=mysql:5.6 --restart=Never mysql-client -- mysql -h accontservice-db -ppassword "
